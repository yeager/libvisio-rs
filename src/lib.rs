...
pub fn parse(path: &str) -> Result<Document> {
    ...
    if XML_EXTENSIONS.contains(&dotted.as_str()) {
        ...
        Err(VisioError::UnsupportedFormat(format!("Unsupported format: .{}", ext)))
    }
}
...
pub fn convert(input_path: &str, output_dir: Option<&str>, page: Option<usize>) -> Result<Vec<String>> {
    let out_dir = output_dir.unwrap_or("/tmp/libvisio_rs_output");
    std::fs::create_dir_all(out_dir)?;

    let doc = parse(input_path)?;
    let basename = Path::new(input_path).file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("visio");
    ...
    for p in &doc.pages {
        if let Some(page_num) = page {
            if p.index != page_num { continue; }
        }

        // Get background shapes if any
        let bg_shapes: Option<Vec<Shape>> = doc.background_map.get(&p.index)
...
pub fn convert_page_to_svg(input_path: &str, page_index: usize) -> Result<String> {
    let doc = parse(input_path)?;
    let page = doc.pages.iter()
        .find(|p| p.index == page_index)
        .ok_or(VisioError::PageNotFound(page_index))?;

    let bg_shapes: Option<Vec<Shape>> = doc.background_map.get(&page.index)
        .and_then(|bg_idx| doc.pages.iter().find(|pp| pp.index == *bg_idx))
        .map(|bg_page| bg_page.shapes.clone());
...
pub fn get_page_info(path: &str) -> Result<Vec<PageInfo>> {
    let doc = parse(path)?;
    Ok(doc.pages.iter().map(|p| PageInfo {
        name: p.name.clone(),
        index: p.index,
        width: p.width,
        height: p.height,
...
pub fn extract_text(path: &str) -> Result<String> {
    ...
    for page in &doc.pages {
        text_lines.push(format!("--- {} ---", if page.name.is_empty() { format!("Page {}", page.index + 1) } else { page.name.clone() }));
...
pub extern "C" fn visio_convert_page_to_svg(doc: *const VisioDocument, page: usize) -> *mut c_char {
    ...
    let bg_shapes: Option<Vec<Shape>> = document.background_map.get(&p.index)
        .and_then(|bg_idx| document.pages.iter().find(|pp| pp.index == *bg_idx))
        .map(|bg_page| bg_page.shapes.clone());

    let svg = render::shapes_to_svg(
        &p.shapes, p.width, p.height,
        &document.masters, &p.connects, &document.media,
        &std::collections::HashMap::new(),
        bg_shapes.as_deref(),
        &document.theme_colors, &p.layers,
    );
...
pub extern "C" fn visio_extract_text(doc: *const VisioDocument) -> *mut c_char {
    ...
    for page in &document.pages {
        lines.push(format!("--- {} ---", if page.name.is_empty() { format!("Page {}", page.index + 1) } else { page.name.clone() }));
        for shape in &page.shapes {
            if !shape.text.is_empty() {
                lines.push(shape.text.clone());
            }
